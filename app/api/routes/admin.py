...
async def run_pipeline() -> RunPipelineResponse:
    """Force a full pipeline run: ingest news → analyze → run the LLM trader.
    Each task already self-chains and triggers the next stage on success, so we
    kick off the head of the chain via Celery canvas to guarantee ordering even
    if upstream stages produce zero rows.
    """
    pipeline = chain(
