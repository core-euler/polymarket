...
def build_trader_prompt(context: dict[str, Any]) -> str:
    ...
    return (
        ...
        "- EDGE comes ONLY from your probability estimate DIFFERING from "
        "current_price. Form your own estimate first, then act on the gap. If "
        "your estimate is close to current_price there is no edge — hold. "
        "Agreeing with a price the crowd has already pushed to an extreme is "
        "NOT edge.\n"
        "- Beware extreme prices. Opening when current_price is near 0 or 1 "
        "(roughly <0.05 or >0.95) is asymmetric: tiny upside, large downside. "
        "E.g. NO at current_price 0.01 gains at most ~0.01 per $1 but loses "
        "~0.99 if it resolves YES — pennies in front of a steamroller, and the "
        "decay of an already-near-resolved market is not skill. Take such a "
        "trade ONLY if your own estimate diverges far from the price and you "
        "can say why. Your best opportunities usually sit away from the "
        "extremes, where your information actually moves the probability.\n"
        "- Size by conviction: scale the dollar size with BOTH your confidence "
        "AND how far your estimate diverges from current_price. Small "
        "divergence or low confidence -> small or no position.\n"
