...
**Status:** active — измерен 2026-05-23 (n=288 / ~5 дней); предиктивный edge подтверждённо ОТСУТСТВУЕТ (см. ### Update 2026-05-23)
**Window:** активация 2026-05-18 → первый замер 2026-05-23 (~5 дней)
...
См. ### Update 2026-05-23 ниже — заполнено по первому замеру.

### Update 2026-05-23 — первый замер v4 (решающий)

**Window:** 2026-05-18 → 2026-05-23 (~5 дней). n = 288 закрытых (UI
показывал 240 — лаг отображения), 3 открытых. Equity $113.01 раскладка
по когортам: v2 +$0.22, v3 +$8.06 (фейковая концентрация v3, «застыла» в
кэше), **v4 +$2.88**, остальное — principal открытых позиций.
**Главное: судим по +$2.88 v4, а не по headline +13%.**

**Воронка / охват (запрос A, сделки за 2026-05-23):**

| market | trades_today | open_now |
|---|---|---|
| Spurs win 2026 NBA Finals | 10 | 0 |
| US invade Iran before 2027 | 10 | 0 |
| US x Iran peace by May 31 | 10 | 0 |
| Strait of Hormuz normal by end May | 10 | 0 |
| Portugal win WC 2026 | 1 | 1 |
| Brazil win WC 2026 | 1 | 1 |
| Trump acquire Greenland | 1 | 1 |

→ 4 рынка упёрлись ровно в cap=10 и отстрелялись досуха; «3 открытых» =
закон Литтла: per-market cap срубил частоту входов ~9× (v3 ~210/день →
v4 ~48/день), книга схлопнулась с ~16 до 3 пропорционально. Низкая
утилизация ($3/$100 locked) — НЕ баг: риск-слой корректно отказывается
прокручивать одни и те же рынки. v3-шные «16 открытых» держались
churn-машиной (распадные рынки переоткрывались за минуты), её и убил cap.

**PnL по дням (запрос B):**
...
daily_loss_limit (−$5) **ни разу не сработал** (худший день −$0.54) →
гипотеза (b) не отвергнута, но и не проверена в бою: хвостового дня типа
v3-05-16 (−$11.56) в окне не было — cap убрал механизм коррелированной
прокрутки, который его и создавал. Guard остаётся непротестированным
страховым.

**PnL по маркетам (запрос C) — РЕШАЮЩЕЕ:**

| market | trades | winrate | total_pnl |
|---|---|---|---|
| Spurs win 2026 NBA Finals | 60 | 66.7% | **+3.636** |
| US invade Iran before 2027 | 60 | 81.7% | **+1.560** |
| Strait of Hormuz normal | 21 | 61.9% | +0.467 |
| Portugal win WC 2026 | 21 | 100% | +0.091 |
| Brazil win WC 2026 | 25 | 16.0% | −0.009 |
| Trump acquire Greenland | 27 | 0.0% | −0.071 |
| **US x Iran peace by May 31** | **60** | **16.7%** | **−2.795** |
| (≈15 рынков по 1 сделке) | 1 | — | 0.000 |

Три независимых доказательства «edge нет»:

**1. Тот же рынок, перевёрнутый знак.** `US x Iran peace by May 31`:
v3 = 371 тр / 94.6% / **+$10.55** (главный герой) → v4 = 60 тр / 16.7% /
**−$2.795** (главный лузер). Один инструмент, один движок, разница в
днях. В v3 движок случайно ехал по направлению распада; сейчас тот же
механизм едет против (вероятно, Иран-новости тикнули вероятность мира
вверх → NO-ставки посыпались). Модель с навыком не флипается с 95% на
17% winrate на том же рынке. Финальное эмпирическое доказательство
фейкового edge, ранее доказанного на уровне кода (Update 2026-05-18).

**2. Бимодальный winrate.** Рынки либо 62–100% (Spurs 67, Iran-2027 82,
Hormuz 62, Portugal 100), либо 0–17% (Iran-May31 17, Brazil 16,
Greenland 0). Зоны навыка (45–58%) НЕТ ни у одного рынка с объёмом.
Сигнатура лотереи на направление дрейфа лонгшота, а не предсказания:
лонгшот либо распадается в твою сторону (выигрываешь почти всё), либо
тикает против (проигрываешь почти всё).

**3. Прибыль = 2 рынка концентрации, остальная книга в минусе.**
Spurs +3.636 + Iran-2027 +1.560 = +$5.196 несут весь PnL; **вся
остальная книга = +2.88 − 5.196 = −$2.32**. v3-паттерн в миниатюре: cap
уменьшил концентрацию (371 → 60 сделок), но не изменил её природу —
движок по-прежнему продаёт NO на распадающихся лонгшотах (Spurs выбывает
по ходу сезона → к нулю; Iran-2027 долгосрочный лонгшот → медленный
распад). Это не news-driven модель, а **harvest распада лонгшотов**
(fade longshot-bias), хрупкий и концентрирующийся.

**Verdict v4 (окончательный по параметрической линии).** Все три цели
риск-слоя достигнуты механически: cap срубил концентрацию (доказано — 4
рынка ровно по 10), correlation guard и daily-stop на месте (последний
не понадобился). **Именно это сделало истинное матожидание видимым:
edge нет.** Гипотеза (a) подтверждена ровно как предсказано — без
conc-маски PnL держится только на 2 остаточных распадных рынках,
остальное отрицательно.

Параметрическая глава закрыта окончательно. Ни cap, ни size, ни порог
edge/confidence не создают предсказательной силы — факт уровня кода
(`edge = strength·confidence·0.5·sign`, сверки с исходом нет; Update
2026-05-18), теперь подтверждённый флипом Iran-May-31. Больше данных
лишь точнее нарисуют тот же ноль.

Развилка (с эмпирическим доказательством на руках, ожидание решения
пользователя):
- **(a)** заменить `_estimate_model_probability` на независимую оценку
  P(событие), систематически бьющую рыночную цену; edge переопределить
  как (независимая оценка − рыночная цена), валидировать против
  резолвов. Единственный путь к реальному edge; обыграть консенсус
  Polymarket трудно.
- **(b)** признать движок безэджевым sentiment/decay-baseline и
  прекратить тюнинг.

Production-код не трогаю без явного выбора пользователя.
...
## v5 — 2026-05-23 — LLM-as-trader

**Status:** в разработке (архитектурный pivot, НЕ параметрическая версия)
**Window:** проектирование 2026-05-23 → …
**Тип:** структурный pivot. **Все** торговые решения принимает LLM;
алгоритмической торговой логики в коде не остаётся.

### Почему pivot, а не v6-параметры

v1–v4 доказали (код-уровнево + эмпирически, см. v3/v4 Verdict): edge
фиктивен по конструкции — `edge = strength·confidence·0.5·sign`,
независимого прогноза P(событие) нет, сверки с исходом нет. Ни один
параметр это не лечит. Решение пользователя 2026-05-23: отдать ВСЕ
решения о действиях (открыть/закрыть/держать, сторона, размер, риск) в
LLM; код = только сбор данных + исполнение.

### Hypothesis

LLM с доступом к свежим новостям + цене + портфелю может принимать
решения, бьющие рыночную цену — там, где формула не могла. **НЕ
предполагаем, что edge появится автоматически**: передача решений в LLM
лишь переносит вопрос. Проверяем строго (см. Протокол валидации).
Возможный исход — тот же «edge нет», но теперь у настоящего forecaster'а,
а не у фейк-формулы; это уже информативно.

### Архитектура (решения пользователя 2026-05-23)

- **Чистый LLM, ноль код-лимитов.** Нет per-market cap / дневного
  стопа / correlation-guard. Риском, размером, концентрацией управляет
  сам LLM. ⚠️ Перед real-money пересмотреть — guardrails вернутся
  ([[project_dev_mode]]).
- **Событийно + батч.** LLM вызывается, когда есть «грязные» рынки
  (новый анализ ИЛИ значимое движение цены с прошлого решения); один
  батч-вызов по всем грязным рынкам.
- **Две модели:** `COMET_MODEL_DEFAULT` (дешёвая) — relevance +
  компрессия новостей; `COMET_MODEL_TRADER` (сильная) — ВСЁ суждение.
  Вариант A: слой анализа понижен до relevance+summary, весь
  causal/forecasting-вывод уехал в трейдер (вычищает последний рудимент
  фейк-формулы — `impact_strength·confidence·0.5`).
- **Единственный код-инвариант — целостность данных:** executor не
  исполняет fill по NULL/битой цене (НЕ риск-лимит, а защита от
  мусорного PnL — баг v1).

Поток:
```
news/snapshots → [trigger: грязные рынки] → [batch context builder]
  → [TRADER LLM → decisions JSON] → [executor: paper_trades + llm_decisions log]
  → [grading против цены на горизонтах + резолва]
```

Удалено из кода (это и был «алгоритм трейдинга»): `signal_engine`
решающая формула (`_estimate_model_probability`/`edge`/`_classify_signal`/
`_evaluate_risk_thresholds` как гейты), `paper_trading.open_eligible_trades`
(edge/direction/v4-caps), `monitor_open_trades` TP/SL/time/jump.
`llm_analysis` остаётся как relevance+summary на дешёвой модели.

### Decision schema (ответ трейдер-LLM)

```json
{
  "decisions": [
    {"market_id": 0, "action": "open|close|hold|adjust",
     "side": "YES|NO", "size": 0.0, "confidence": 0.0,
     "rationale": "почему"}
  ],
  "portfolio_notes": "оценка экспозиции/концентрации"
}
```

### Протокол валидации (хребет — без него повторим v3)

- Таблица `llm_decisions`: ts, market, цена в момент решения, полный
  ответ (action/side/size/confidence/rationale), prompt/response trace,
  strategy_config_id; поля grading (цена на +1h/+4h/+24h, резолв)
  дозаполняются скорером позже.
- **Калибровка:** confidence 0.8 → выигрыш ~80%?
- **Бьём ли рынок:** PnL с ИЗОЛИРОВАННЫМ decay-harvest (выносим
  лонгшот-распадные рынки, как Иран/Spurs в v3/v4). Если без них ≤0 и
  калибровка плоская → edge нет, тот же вердикт.
- Следим за распределением по рынкам — не воссоздаёт ли LLM
  концентрацию сам (код больше не мешает).

### Token-бюджет

Драйвер расхода — **трейдер** (контекст-тяжёлый: портфель + до ~16
рынков, reasoning-output), не анализ (много дешёвых мелких вызовов).
Рычаги: prompt caching статичного каркаса, дельты новостей,
анализ-как-компрессор, тугой батч, ограничение длины rationale.

### Параметры / схема

Схема создаётся через `Base.metadata.create_all` (alembic в проекте
НЕТ; новая таблица `llm_decisions` появляется при следующем init/wipe;
`paper_trades.signal_id` становится nullable → **требует wipe БД**,
который в test-фазе штатный, seed регенерирует). seed.py → v5:
`parameters_json`/`paper_trading_rules_json` почти пустые (порогов/каппов
нет — конфиг переехал в промпт/модель).

### Verdict

(in-flight — заполнить после первого прогона LLM-трейдера: калибровка +
PnL с изолированным decay-harvest)
