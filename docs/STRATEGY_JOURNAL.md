...
### Verdict — 2026-05-24 (n=18 направленных, +1h/+4h; resolved=0)

**ОТРИЦАТЕЛЬНЫЙ edge, не просто отсутствие.** Multi-horizon scorer:

- **core (p0 ∈ (0.15,0.85))**: favorable ~30% (+1h 29.4% / −0.057;
  +4h 31.2% / −0.072). Рынок уходит ПРОТИВ core-ставок ~70% времени.
- **Калибровка ПЕРЕВЁРНУТА и почти значима**: полоса высокой уверенности
  [0.70,0.85] = 10% fav @+1h (1 из 10, p≈1.1%) / 0% @+4h (0 из 9, p≈0.2%),
  средний ход −0.105 / −0.148. Средняя полоса [0.50,0.70] ≈ 57% (≈монета).
  Модель **наиболее неправа там, где наиболее уверена** — это анти-сигнал,
  не шум.
- **Headline equity — шум**: тот же когорт прыгал $85.66 (−14.3%) →
  $108.33 (+8.3%) за неделю на n=18 (лучший +12.68 / худший −15.20).
  Игнорируем — это ровно ловушка v1–v4.
- +24h n=3 (100% fav) — статистически ноль, не читать.

Глава **LLM-as-trader ЗАКРЫТА отрицательно** (4-й негатив подряд). Не
нейтральна — уверенно неправа.

### Коррекция вердикта v3/v4 «fake edge» (push-back пользователя, валиден)

Я склеил два разных утверждения: **(a)** clamp-разметка PnL артефактна
[реальный баг] vs **(b)** входы fade-the-longshot, к которым clamp вёл, не
имеют реализованного edge [**не было чисто протестировано к резолюции**].
FLB (favorite-longshot bias) литературно реален; v3 Iran +$10.55 / Spurs
+$3.80 могут быть настоящим FLB-харвестом, а не чистым артефактом — но 2
победителя = survivorship, недоказано.

Ключевая математика (дисциплинирует тезис): NO по цене p платит **+p** с
вероятностью (1−q) и **−(1−p)** с q ⇒ EV = **p − q** ⇒ +EV только если
лонгшот переоценён (p > q_true). Edge ограничен размером мисприсинга,
обязан пережить **спред**; редкий YES-флип — это всё распределение
дисперсии. «Лечение правилами, чтобы увернуться от хвоста» само себя
подрывает (= протаскивает обратно мёртвый forecasting-skill).

### v6 — explicit FLB harvest (план, код НЕ начат как стратегия)

Сначала **survivorship-free pre-mortem**, не торговый код:

1. Тянем ВСЕ resolved бинарные рынки Polymarket за окно (Gamma
   `closed=true` + CLOB `prices-history`).
2. Ex-ante триггер (без look-ahead): первая точка, где YES-цена ∈
   [0.05,0.15] И до резолюции (7d, 90d]; calendar-resolved; volume/liq ≥ X.
3. Мерим к **РЕАЛЬНОЙ резолюции** (исход YES/NO из `outcomePrices`), не к
   марку — измерение не-исхода и было ошибкой v1–v5.
4. Метрика: средний PnL/контракт = (p̄ − q̄) минус спред; max drawdown по
   кластеризации резолв-событий (Iran-as-event, не -market).
5. Первый прогон **БЕЗ LLM-фильтра** (сырой FLB) — чтобы не протащить
   назад мёртвый claim-A. LLM-фильтр новостей добавляем только если сырой
   уже +EV.

Решение: сырой FLB +EV после спреда при переживаемом drawdown →
настоящий v6. Нет → публикуем negative result, теперь честно изолированный.
Харнес: `app/research/flb_replay.py` (offline, без записи в БД).

### v6 — ВЕРДИКТ pre-mortem (2026-05-25): ФАЛЬСИФИЦИРОВАН

Survivorship-free replay, scored к РЕАЛЬНОЙ резолюции (`outcomePrices`), без
LLM-фильтра. Оценщик: PnL/контракт фейда = p̄ − q (минус спред); +EV только если
лонгшоты систематически переоценены. Три прогона:

| прогон | вселенная / вход | n | p̄ | q (выбило YES) | gross | @spread 0.02 |
|---|---|---|---|---|---|---|
| исходный | ликвид, earliest-in-band | 48 | 0.104 | **0.208** | −0.105 | −0.11 |
| Check 2 | ликвид, **stable ≥14d** | 52 | 0.081 | 0.096 | −0.015 | −0.025 |
| Check 1 | **illiquid 2k–50k** | 58 | 0.111 | 0.103 | +0.008 | −0.002 |

Торгуемого FLB нет:
- **Ликвид (где можно разместить капитал) — анти-FLB**, минус на контракт.
  Фильтр стабильности (steelman календарного распада, отсев news) РЕАЛЬНО
  срезал q с 0.208 → 0.096 — но приземлил на справедливую цену (q≈p), не выше.
  Премии под спокойными лонгшотами нет: их не переоценивают.
- **Illiquid-хвост** — шёпот FLB (+0.78% gross), как и предсказывает теория
  (шарпам лень чинить мелкие рынки), но (а) статистически ноль (edge ≈ 1/5 σ
  при n=58), (б) экономически мёртв: положителен лишь при нулевом спреде, а
  реальные спреды в этих рынках 5–15c — при 4c уже −1.2%.

YES-хиты даже после stable-фильтра остались news/event-driven (Russia-Ukraine
ceasefire ×2, ETH ETF, Epstein/Colbert) — «отфильтруем новости → увернёмся от
хвоста» не работает: хвост И ЕСТЬ стабильные лонгшоты.

Итог: v6 fade-the-longshot закрыт ДО единой строчки торгового кода. Параметрическая
И LLM-as-trader И FLB-harvest главы — все три дали no/negative edge. Полный отчёт:
`docs/research/flb_pre_mortem.md`.
