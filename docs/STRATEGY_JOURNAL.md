...
(in-flight — заполнить через 24-48ч)

### Results

(in-flight — заполнить через 24-48ч)

**Ожидание для замера через 24-48ч (рабочая гипотеза):**
- close_reason: TP ≥ 15%, SL ≥ 15%, остальное time_limit
- avg PnL ≥ +$0.0030 (vs v2 +$0.0008)
- winrate в диапазоне 50-60%
- при тех же 16 маркетах — ~50-80 закрытых трейдов в сутки

Если TP по-прежнему 0 → пороги всё ещё слишком высокие, опускаем до
0.01 / 0.0075 в v4. Если SL>30% → агрессивный risk, поднимаем стоп.

### Verdict

(заполнить после bump'а на v4 или ≥ 100 закрытых трейдов)
